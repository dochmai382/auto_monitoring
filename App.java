...
/*
        ...
        HttpClient client = HttpClient.newHttpClient(); // 클라이언트 생성
...
//            HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
            ...
            String response = getDateFromAPI("news.json", keyword, display, start, sort);
            ...
            String imageResponse = getDateFromAPI("image", keyword, display, start, sort);
//            logger.info(imageResponse);
            // 2. 이미지
            imageLink = imageResponse.split("link\":\"")[1].split("\",")[0].replace("\\", "");
            logger.info(imageLink);
            HttpRequest request = HttpRequest.newBuilder().uri(URI.create(imageLink)).build();
            String[] tmp2 = imageLink.split("\\.");
            Path path = Path.of("%d_%s.%s".formatted(new Date().getTime(), keyword, tmp2[tmp2.length - 1]));
    ...
    private String getDateFromAPI(String path, String keyword, int display, int start, SortType sort) throws Exception {
//        String url = "https://openapi.naver.com/v1/search/news.json";
